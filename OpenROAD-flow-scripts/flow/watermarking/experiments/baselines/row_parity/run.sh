...
#
# Optional env:
#   FLOW_VARIANT    output variant (default: baseline-row-parity)
...
export FLOW_VARIANT="${FLOW_VARIANT:-baseline-row-parity}"

WM_DIR="${WM_HOME}"

ts()  { date '+%Y-%m-%d %H:%M:%S'; }
log() { echo "[$(ts)] [row_parity/run.sh] $*"; }
...
OUT_ODB="${OUT_RES}/3_place_row_parity.odb"
EMBED_CSV="${OUT_RES}/row_parity_embed.csv"
...
if [[ -f "${DRT_ODB}" ]]; then
  ${OPENROAD_EXE} -python -exit "${SCRIPT_DIR}/verify.py" \
    ...
    --embed-csv "${EMBED_CSV}" \
    --stage DRT \
    --out-csv "${OUT_RES}/row_parity_verify_DRT.csv"
else

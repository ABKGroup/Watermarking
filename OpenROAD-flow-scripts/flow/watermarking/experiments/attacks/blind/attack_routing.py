...
``routing_wm/run_attack_route.sh`` which loads the watermarked 4_cts.odb, tags
every WM net normally, *clears* the watermark tag on the selected attack
subset, and re-runs detail_route -- so the attacked nets are routed without the
wrong-way penalty.

Env:
    WM_ROUTE_QR_IN      per-net (ww_len, tot_len) CSV from tools/dump_route_qr.py,
                        used to define the eligible set (tot_len > 0)
...
def main() -> int:
    in_csv = os.environ["WM_ROUTE_QR_IN"]

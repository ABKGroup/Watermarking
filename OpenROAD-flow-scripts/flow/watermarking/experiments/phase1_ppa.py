...
- Baseline methods (Row-parity, Buffer-insertion, ICMarks) also use the experiment
...
#
# Sources:
#   PDMarks placement:  placement_wm/wm_log/<design>_run_place_wm_ordering_*.log
#                       line: "[run_place_wm] finished status=0 elapsed=<N>s"
#   PDMarks CTS:        cts_wm/wm_log/<design>_run_cts_wm_*.log
...
PLACE_WM_LOG_DIR = FLOW_HOME / "watermarking" / "placement_wm" / "wm_log"
CTS_WM_LOG_DIR   = FLOW_HOME / "watermarking" / "cts_wm"         / "wm_log"
...
def _wm_runtime_baseline(method: str, platform: str, design: str,
    ...
    found."""
    name_map = {"Row-parity": "row_parity",
...
def _routing_p(ref_results_dir: Path, design_id: str):
    """Read the routing stat from route_qr.csv + watermark_nets.txt in a results dir.

    ``design_id`` seeds the randomization null (see lib/route_stat.design_seed).
    """
    ...
    qr_names = (
        "route_qr.csv",
        "route_qr_5_route.csv",
        "route_qr_6_final.csv",
    )
    rc = next((ref_results_dir / name for name in qr_names
               if (ref_results_dir / name).exists()), ref_results_dir / "route_qr.csv")
    if not rc.exists():
    ...
    if not rc.exists():
        return None, None
    counts = per_net_qr(rc)
    wm_set = read_watermark_nets(wm)
    st = route_stat_from_qr(counts, wm_set, design_id)
    return st.p_R, st
...
def collect_one(b, method: str):
    ...
    if method == "P-only":
        ...
        out["dPower"]   = _delta(ref.power_w,   cur.power_w)
        out["dRuntime"] = _delta(ref.runtime_s, cur.runtime_s)
        pR, _ = _routing_p(experiment_results(plat, design, var), design)
        if pR is not None:
            out["p_R"] = pR
            pc_parts.append(pR)
        else:
        ...
        if X_C > 0:
            pc_parts.append(pc_stage(X_C, x_C, 0.5))
        pR, _ = _routing_p(ppa_res, design)
        if pR is not None:
            out["p_R"] = pR
            pc_parts.append(pR)
        else:
